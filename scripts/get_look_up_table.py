...
#                                     'stat':'NA', 'p_val': 'NA'}, ignore_index=True)
		    ...
		    'group': group, 'color_1': color_1, 'color_2': color_2,
		    'var_name': var, 'len_x': len(x), 'len_y': len(y),
		    'mean_x': x.mean(), 'mean_y': y.mean(),
		    'stat': res.statistic, 'p_val': res.pvalue
            }])
...
look_up_p.to_csv("data/look_up_p.csv", index=False)
print("Saved look_up_p.csv")
