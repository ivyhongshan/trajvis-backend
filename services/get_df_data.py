...
def _resolve_path(filename: str) -> str:
    """
    ?????????
    1) ????? DATA_BUCKET???? gs://<bucket>/<prefix><filename>
    2) ??????? data/<filename>
    ????????? PATH_<NAME> ???????????? PATH_CKD=gs://.../ckd_emr_data.csv?
    """
    # ??? override??? PATH_CKD?PATH_DEMO ...
    overrides = {
        "ckd_emr_data.csv": os.getenv("PATH_CKD"),
        "ckd_crf_demo.csv": os.getenv("PATH_DEMO"),
        "ordered_feats.csv": os.getenv("PATH_ORDERED_FEATS"),
        "features_all_csn_id.csv": os.getenv("PATH_FEATURES_ALL_CSN"),
        "cal_risk.csv": os.getenv("PATH_CAL_RISK"),
        "look_up_p.csv": os.getenv("PATH_LOOK_UP_P"),
    }
    if filename in overrides and overrides[filename]:
        return overrides[filename]

    bucket = os.getenv("DATA_BUCKET")
    prefix = os.getenv("DATA_PREFIX", "data/")
    if bucket:
        return f"gs://{bucket.rstrip('/')}/{prefix}{filename}"
...
def _load_csv(filename: str, **kwargs) -> pd.DataFrame:
    ...
    # ????????????? & skipinitialspace
    defaults = dict(skipinitialspace=True, delimiter=",")
    defaults.update(kwargs)
    return pd.read_csv(path, **defaults)

# ???? DataFrame ??????????????
def ckd_data_df() -> pd.DataFrame:
    return _load_csv("ckd_emr_data.csv")

def ckd_crf_demo_df() -> pd.DataFrame:
    return _load_csv("ckd_crf_demo.csv")

def ordered_feats_df() -> pd.DataFrame:
    return _load_csv("ordered_feats.csv")

def features_all_csn_df() -> pd.DataFrame:
    return _load_csv("features_all_csn_id.csv")

def acr_df_pats_df() -> pd.DataFrame:
    return _load_csv("cal_risk.csv")

def look_up_p_df() -> pd.DataFrame:
    return _load_csv("look_up_p.csv")

# ---------- ???????????????? ----------
def get_pat_kidney_risk(id):
    df = acr_df_pats_df()
    return df[df['pat_id'] == id]

def get_pat_records(id):
    df = ckd_data_df()
    return df[df['pat.id'] == id]
...
def get_pat_unique_concept(id):
    df = get_pat_records(id).sort_values(by=['age'])
...
def getLabTestViewData(pat_id):
    ...
    for concept_pair in concept_age_val:
        ...
        for age_val_pair in concept_pair[1]:
            age = int(age_val_pair[0]); val = age_val_pair[1]
...
def getIndicatorMarkers(pat_id):
    features_all_csn = features_all_csn_df()
    look_up_p = look_up_p_df()
        ...
        if len(one_age_visit) > 0:
            for _, row in one_age_visit.iterrows():
                ...
                if group is not None:
                    mark = 'predict' if group in ('before_1', 'before_2') else 'marker'
...
def label_category(label):
    if label in before_1: return 'before_1'
    if label in before_2: return 'before_2'
    if label in after_green_after_blue: return 'after_green_after_blue'
    if label in after_orange_after_green_blue: return 'after_orange_after_green_blue'
    return None
...
def toIntergers(data): return np.trunc(data).astype(int)
def range_list(a, b): return list(range(a, b + 1))

normal_range_dict = {
    'EGFR':[60, 200], 'TBIL': [0.1, 1.2], 'BP_DIASTOLIC': [60, 80], 'BP_SYSTOLIC': [90, 120],
    'WT': [90, 220], 'HT': [57, 78], 'CHOLESTEROL': [50, 200], 'CREATINE_KINASE': [22, 198],
    'HEMOGLOBIN': [11.6, 17.2], 'INR': [0.8, 1.1], 'ALT_SGPT': [7, 56], 'AST_SGOT': [8, 45],
    'ALK': [44, 147], 'HDL': [40, 100], 'LDL': [40, 100], 'TRIGLYCERIDES': [20, 150],
    'HBA1C': [4, 6.5], 'TROPONIN': [0, 0.04]
}
...
def getHierarchicalClusterInput(pat_id):
    ...
    return [vect_dict[key] for key in vect_dict]
...
def getOrderofConcepts(pat_id):
    ...
    if len(matrix) == 1:
        ...
        key_tuples = [new_key(concepts[i], labels[i]) for i in range(len(concepts))]
